...
export async function GET(request: Request) {
  ...
  try {
    ...
    // Fetch all messages in the date range with author info
    ...
    while (hasMore) {
      ...
      if (error) {
        console.error('[analytics/top-contributors] Query error:', error)
        throw error
      }
    ...
    return NextResponse.json({ data: topContributors })
  } catch (error: any) {
    console.error('[analytics/top-contributors] GET error', error)
